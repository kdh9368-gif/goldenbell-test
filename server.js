...
if(m.type==='join'){if(state.phase!=='lobby')return send(ws,'error',{message:'게임이 이미 시작되었습니다.'});const name=String(m.name||'').trim().slice(0,30);if(!name)return send(ws,'error',{message:'이름 또는 참가번호를 입력해주세요.'});state.participants.set(ws.id,{name,alive:true});return broadcast()}
